      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2">
          ...
          <div
            ...
            class="grid grid-cols-2 gap-4 mb-8 md:grid-cols-3"
          >
            <div
            ...
            >
              ...
              <div class="relative overflow-hidden bg-gray-100 aspect-square">
                ...
                <!-- Actions -->
                <div
                  class="absolute inset-0 flex items-center justify-center gap-2 transition-opacity duration-300 opacity-0 group-hover:opacity-100 bg-black/50"
                >
                  <button
                    @click.stop="goToProductDetail(item.product || item.bundle)"
              ...
              <div class="p-3 space-y-1.5">
                <p class="text-xs tracking-wide text-gray-500 uppercase">
                  {{ item.product?.brand || item.bundle?.brand || "Brand" }}
                </p>

                <h3
                  @click="goToProductDetail(item.product || item.bundle)"
...
const sortedWishlistItems = computed(() => {
  ...
  switch (sortBy.value) {
    ...
    case "rating":
      items.sort(
        (a, b) =>
          (b.product?.ratings?.average || 0) -
          (a.product?.ratings?.average || 0)
      );
      break;
  }
...
const goToProductDetail = (entity) => {
  if (!entity) return;
  // If it's a bundle (has bundlePrice or items), route to bundle page
  if (entity.bundlePrice || entity.items) {
    router.push(`/bundles/${entity._id}`);
    return;
  }
  router.push(`/products/${entity.slug || entity._id}`);
};

const addToCheckout = (wishlistItem) => {
  const id = wishlistItem.product?._id || wishlistItem.bundle?._id;
  const exists = selectedCheckoutItems.value.some(
    (item) => (item.product?._id || item.bundle?._id) === id
  );
  if (!exists) {
...
const removeFromCheckout = (id) => {
  selectedCheckoutItems.value = selectedCheckoutItems.value.filter((item) => {
    const pid = item.product?._id;
    const bid = item.bundle?._id;
