  ...
  <section class="relative w-full min-h-screen bg-gray-50">
    ...
    <div class="px-2 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div v-else-if="error" class="py-12 text-center">
        <div
          class="max-w-md p-6 mx-auto bg-white border border-red-200 rounded-lg shadow-sm"
        >
          <svg
      ...
      <div v-else-if="product._id">
        <Transition name="fade">
          <div
            v-if="successMessage"
            class="fixed z-50 p-4 text-white bg-green-500 rounded-lg shadow-lg top-4 right-4"
          >
            {{ successMessage }}
          </div>
        </Transition>
        ...
        <Transition name="fade">
          <div
            v-if="errorMessage"
            class="fixed z-50 p-4 text-white bg-red-500 rounded-lg shadow-lg top-4 right-4"
          >
            {{ errorMessage }}
          </div>
        </Transition>
...
const activeSale = computed(() => {
  ...
  const sortedPrograms = [...activeSalePrograms.value]
    ...
    .sort(
      (a, b) =>
        (b.benefits?.discountPercentage || 0) -
        (a.benefits?.discountPercentage || 0)
    );
...
const breadcrumbItems = computed(() => {
  ...
  // If coming from sale program, add it to breadcrumb
  if (saleProgramIdFromUrl.value && activeSale.value) {
    items.push({
      label: "Sale Programs",
      to: "/sale-programs",
    });
    items.push({
      label: activeSale.value.title || "Sale Program",
      to: `/sale-programs/${saleProgramIdFromUrl.value}/products`,
    });
  } else if (product.value.category && Array.isArray(product.value.category)) {
    const buildCategoryChain = () => {
      const categories = product.value.category;

      // Find the deepest category (highest level number or one without being a parent)
      let deepestCategory = categories[0];
      for (const cat of categories) {
        if (
          cat.level !== undefined &&
          cat.level > (deepestCategory.level || 0)
        ) {
          deepestCategory = cat;
        }
      }

      // Build chain from root to this category
      const chain = [];
