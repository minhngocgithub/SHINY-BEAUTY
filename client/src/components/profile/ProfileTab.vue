<template>
  <div class="space-y-6">
    ...
    <div
      class="overflow-hidden border border-gray-100 shadow-lg bg-white/90 backdrop-blur-xl rounded-3xl"
    >
      <div
        class="relative h-32 bg-gradient-to-r from-rose-200 via-violet-200 to-blue-200"
      >
      ...
      <div class="relative p-6 sm:-mt-16">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end">
          <ModelChangeAvatar
            :currentAvatar="
              authStore.user?.avatar || { url: '', public_id: '' }
            "
          ...
          <div class="flex-1 text-center sm:text-left">
            ...
            <div class="flex justify-center gap-2 mt-2 sm:justify-start">
              <span
                class="px-3 py-1 text-sm border rounded-full bg-emerald-50 text-emerald-700 border-emerald-200"
              >
                Member{{
                  authStore.user?.createAt
                    ? " since " + formatYear(authStore.user.createAt)
                    : ""
                }}
              </span>
              <span
                v-if="authStore.user?.isAdmin"
                class="px-3 py-1 text-sm border rounded-full bg-violet-50 text-violet-700 border-violet-200"
                >Admin</span
              >
            </div>
          </div>
    ...
    >
      ...
      <div class="grid gap-6 md:grid-cols-2">
        ...
        <div class="space-y-2">
          ...
          <div
            v-else
            class="px-4 py-3 text-gray-700 border border-gray-100 bg-gray-50 rounded-xl"
          >
        ...
        <div class="space-y-2">
          ...
          <input
            ...
            :class="[
              'w-full px-4 py-3 transition-all duration-200 border rounded-xl',
          ...
          <p
            v-if="isEditing && authStore.user?.isOAuthUser"
