<template>
  <div class="space-y-4 add-to-cart-section">
    ...
    <div class="flex items-center gap-4">
      ...
      <div class="flex items-center border rounded-lg">
        <div class="flex items-center w-24 border border-gray-300 rounded">
          <button
            ...
            :disabled="quantity <= 1"
            class="flex-shrink-0 p-2 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <svg
      ...
      <span class="text-sm text-gray-500"> {{ maxQuantity }} available </span>
    </div>
    ...
    <div
      ...
      class="p-3 text-sm text-red-600 border border-red-200 rounded-lg bg-red-50"
    >
      {{ cartStore.error }}
    </div>
    ...
    <div
      ...
      class="p-3 text-sm text-green-600 border border-green-200 rounded-lg bg-green-50"
    >
      {{ successMessage }}
    </div>
  </div>
...
const maxQuantity = computed(() => {
  return Math.min(props.product.countInstock || 0, 10); // Max 10 per order
});
...
const handleAddToCart = async () => {
  try {
    ...
    if (result.success) {
      ...
      successMessage.value = result.message || "Added to cart successfully!";
...
// Watch for cart store error changes
watch(
  () => cartStore.error,
  (newError) => {
    if (newError) {
