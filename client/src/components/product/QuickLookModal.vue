        ...
        >
          <div class="flex flex-col lg:flex-row">
            ...
            <div class="flex flex-col p-6 lg:w-1/2">
              ...
              <div class="mb-6">
                <!-- Sale Price -->
                <div v-if="hasBackendSale" class="flex items-baseline gap-3">
                  ...
                  >
                  <span
                    class="px-2 py-1 text-xs font-semibold text-red-600 bg-red-100 rounded"
                    >-{{ product.discountPercentage }}% OFF</span
                  >
                </div>
                <!-- Regular Price -->
                <span v-else class="text-2xl font-bold text-gray-900"
                  >${{ product.price?.toFixed(2) || "145.00" }}</span
                >
              </div>
              <div class="mb-4">
                <div
...
const productImages = computed(() => {
  ...
  if (
    ...
    const sortedImages = [...props.product.image].sort((a, b) => {
...
// ✅ Check if product has backend-applied sale
const hasBackendSale = computed(() => {
  return (
    props.product.hasSale === true &&
    props.product.finalPrice > 0 &&
    props.product.finalPrice < props.product.price
  );
});

// ✅ Display price (sale or regular)
const displayPrice = computed(() => {
  if (hasBackendSale.value) {
