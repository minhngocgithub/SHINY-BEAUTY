...
export const useWishlistStore = defineStore('wishlist', () => {
    ...
    const addBundleToWishlist = async (bundle) => {
        try {
            ...
            if (!authStore.state.isLoggedIn) {
                // Could store bundle locally as a bundle entry
                const localWishlist = getLocalWishlist()
                const exists = localWishlist.some(i => i.bundle && i.bundle._id === bundle._id)
                if (!exists) {
                    localWishlist.push({
                        _id: `local_bundle_${Date.now()}`,
                        bundle,
                        itemType: 'bundle',
                ...
                return { success: true, message: 'Added bundle to wishlist (local)' }
            }

            const response = await addBundleToWishlistApi(bundle._id)

            if (response.data.success) {
                wishlistItems.value = response.data.wishlist?.items || []
                console.log('Bundle added to wishlist. Total:', wishlistItems.value.length)
                return { success: true, message: response.data.message || 'Added to wishlist successfully' }
            }
        } catch (err) {
            const message = err.response?.data?.message || err.message || 'Failed to add bundle to wishlist'
            error.value = message
            console.error('Add bundle to wishlist error:', err)
            throw err
        } finally {
            loading.value = false
        }
        ...
        try {
            ...
            if (response.data.success) {
                wishlistItems.value = wishlistItems.value.filter(item => {
                    ...
                    if (item.bundle && item.bundle._id) return item.bundle._id !== productId
                    return true
                })
    ...
    const isInWishlist = (id) => {
        return wishlistItems.value.some((item) => {
            if (item.product && item.product._id) return item.product._id === id
            if (item.bundle && item.bundle._id) return item.bundle._id === id
            if (item.productId) return item.productId === id
            return false
        })
    }
