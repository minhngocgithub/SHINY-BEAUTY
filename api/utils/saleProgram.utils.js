...
class SaleProgramService {
    static async getActiveSalePrograms({ user = null, cart = null, now = new Date() } = {}) {
        try {
            // Don't use .lean() to preserve virtual fields (isCurrentlyActive, timeRemaining)
            ...
            // Convert to plain objects while preserving virtuals
            const programsWithVirtuals = salePrograms.map(sp => {
                const obj = sp.toObject();
                return obj;
            });

            return programsWithVirtuals.filter(sp => this.validateProgramConditions(sp, user, cart));
        } catch (error) {
    ...
    static isProductEligible(program, product) {
        ...
        if (conditions.excludeProducts?.some(id => id.toString() === productId)) {
            return false;
        }

        // ✅ FIXED: Check if product matches ANY of the conditions (OR logic, not AND)
        let hasApplicableProducts = conditions.applicableProducts?.length > 0;
        let hasCategories = conditions.categories?.length > 0;
        let hasBrands = conditions.brands?.length > 0;

        // If program has NO conditions at all, applies to all products
        if (!hasApplicableProducts && !hasCategories && !hasBrands) {
            return true;
        }

        // Check specific products
        if (hasApplicableProducts &&
            conditions.applicableProducts.some(id => id.toString() === productId)) {
            return true;
        }
        ...
        if (hasCategories) {
            ...
            const conditionCategoryIds = conditions.categories.map(c => c.toString());

            if (conditionCategoryIds.some(condCat => productCategoryIds.includes(condCat))) {
                return true;
            }
        }
        if (hasBrands && conditions.brands.includes(product.brand)) {
            return true;
        }
        return false;
    }
