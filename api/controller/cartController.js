...
const addToCard = async (req, res) => {
    try {
        const { productId, bundleId, quantity = 1, replaceMode = false } = req.body
        ...
        if (productId) {
            ...
            if (existingItem) {
                // If replaceMode=true (e.g. from cart merge), SET quantity instead of ADD
                const newQuantity = replaceMode ? quantity : (existingItem.quantity + quantity)
        ...
        if (bundleId) {
            ...
            if (existingBundle) {
                // If replaceMode=true, SET quantity instead of ADD
                const newQuantity = replaceMode ? quantity : (existingBundle.quantity + quantity);
        ...
        const finalItem = productId
            ? updatedUser.cartItems.find(item => item.product?._id?.toString() === productId)
            : updatedUser.cartItems.find(item => item.bundle?._id?.toString() === bundleId)
