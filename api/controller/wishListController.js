...
const getWishlist = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId)
            .populate({ path: 'wishlistItems.product', select: 'name brand price salePrice image countInstock ratings discountPercentage isOnSale' })
            .populate({ path: 'wishlistItems.bundle', select: 'name description image items bundlePrice originalPrice discountPercentage' })
            .lean();
...
const addToWishlist = async (req, res) => {
    try {
        ...
        if (bundleId) {
            bundle = await ProductBundle.findById(bundleId).populate('items.product');
            if (!bundle) {
        ...
        // Check if already in wishlist (product or bundle)
        const exists = user.wishlistItems.some((item) => {
            if (product && item.product) return item.product.toString() === productId;
            if (bundle && item.bundle) return item.bundle.toString() === bundleId;
            return false;
        });
        ...
        if (bundle) {
            // For a bundle, snapshot priceWhenAdded from bundle.bundlePrice
            user.wishlistItems.push({
                bundle: bundleId,
                itemType: 'bundle',
                priceWhenAdded: bundle.bundlePrice || 0,
                addedAt: new Date()
            });
        }
        ...
        const updatedUser = await User.findById(userId)
            .populate({ path: 'wishlistItems.product', select: 'name brand price salePrice image countInstock ratings' })
            .populate({ path: 'wishlistItems.bundle', select: 'name description image items bundlePrice originalPrice discountPercentage' })
            .lean();
...
const removeFromWishlist = async (req, res) => {
    try {
        ...
        if (!productId) {
            return res.status(400).json({ success: false, message: 'Product or bundle ID is required' });
        }
        ...
        // Remove matching product or bundle entries
        user.wishlistItems = user.wishlistItems.filter((item) => {
            if (item.product) return item.product.toString() !== productId;
            if (item.bundle) return item.bundle.toString() !== productId;
            return true;
        });
...
const checkInWishlist = async (req, res) => {
    try {
        ...
        const inWishlist = user.wishlistItems.some((item) => {
            if (item.product) return item.product.toString() === productId;
            if (item.bundle) return item.bundle.toString() === productId;
            return false;
        });
